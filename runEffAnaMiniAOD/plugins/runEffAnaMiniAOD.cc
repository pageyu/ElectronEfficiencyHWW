...
runEffAnaMiniAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if (abs(selLeptons[0].pdgId()) == 11 && abs(selLeptons[1].pdgId()) == 11){
    ...
    bool passTagISO(false)       , passProbeISO(false);
    bool TagTightIdSTD(false)    , ProbeTightIdSTD(false);
    bool TagTightMVAIdSTD(false) , ProbeTightMVAIdSTD(false);
    //bool TagMVAEle(false)        , ProbeMVAEle(false);
    //bool TagCBEle(false)         , ProbeCBEle(false);
...
//       passProbeISO = ecalPFIsos < 0.45 && hcalPFIsos < 0.25 && trackIsos < 0.2;
        ...
        TagTightMVAIdSTD    = (*mva_tight_id_decisions)[electronsHandle->ptrAt(first)]   && electronMVATightIdExtra.passId(selLeptons[first].el);
        ProbeTightMVAIdSTD  = (*mva_tight_id_decisions)[electronsHandle->ptrAt(second)]  && electronMVATightIdExtra.passId(selLeptons[second].el);
        ...
        if (passTagEleKin && TagTightIdSTD && passTagdRCut && passTagISO){
            TagTightIdSTD = true;
        }

        if (passTagEleKin && TagTightMVAIdSTD && passTagdRCut && passTagISO){
            TagTightMVAIdSTD = true;
        }

        TagEle = TagTightMVAIdSTD || TagTightIdSTD;
        if (!TagEle){
            return;
        }
        ...
        tagEta = etaf;
        tagPhi = phif;
        tagMVA = TagTightMVAIdSTD;
        tagCB  = TagTightIdSTD;

        //Selection of the Probe
        bool ProbeEle = false;
        passProbeEleKin = ((abs(etas) >= 0 && abs(etas) <= 2.5) && pts > 0);    //Fix it       
        if ((ProbeTightIdSTD && passProbeEleKin && passProbeISO)){
            ProbeTightIdSTD = true;
        }
        if ((ProbeTightMVAIdSTD && passProbeEleKin && passProbeISO)){
            ProbeTightMVAIdSTD = true;
        }

        ProbeEle = ProbeTightMVAIdSTD || ProbeTightIdSTD;
        probeMVA = ProbeTightMVAIdSTD;
...
b_tagMVA              = ntuple->Branch("tagMVA"            ,&tagMVA             , "tagMVA/D");
b_tagCB               = ntuple->Branch("tagCB"             ,&tagCB              , "tagCB/D");
b_probeMVA            = ntuple->Branch("probeMVA"          ,&probeMVA           , "probeMVA/D");
